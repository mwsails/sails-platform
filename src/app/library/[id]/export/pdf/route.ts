...
import { buildOnePagerPdf, type OnePager } from "@/lib/library/one-pager-export";
...
/**
 * A plain GET route, same reasoning as /playbook/export — a real file
 * download needs Content-Disposition on the response itself.
 */
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const buffer = await buildOnePagerPdf(onePager, {
  ...
  const filename = `${slugify(companyName || onePager.headline)}-one-pager.pdf`;
